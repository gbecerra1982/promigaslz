...
@description('Subnet name for Azure AI Foundry agents.')
@minLength(1)
param agentSubnetName string
@description('Subnet name dedicated to private endpoints.')
@minLength(1)
param peSubnetName string
@description('Subnet name for VPN or ExpressRoute gateways.')
@minLength(1)
param gatewaySubnetName string
@description('Subnet name reserved for Azure Bastion hosts.')
@minLength(1)
...
@description('Subnet name for Azure Application Gateway.')
@minLength(1)
param azureAppGatewaySubnetName string
@description('Subnet name for administrative jumpbox virtual machines.')
@minLength(1)
param jumpboxSubnetName string
@description('Subnet name for Azure Container Apps environment integration.')
@minLength(1)
param acaEnvironmentSubnetName string
@description('Subnet name for Azure DevOps self-hosted build agents.')
@minLength(1)
param devopsBuildAgentsSubnetName string
@description('Subnet name dedicated to PostgreSQL flexible servers.')
@minLength(1)
param psqlSubnetName string

@description('CIDR prefix for the agent subnet.')
param agentSubnetPrefix string
@description('CIDR prefix assigned to the private endpoint subnet.')
param peSubnetPrefix string
@description('CIDR prefix for VPN or ExpressRoute gateway subnet.')
param gatewaySubnetPrefix string
@description('CIDR prefix reserved for Azure Bastion subnet.')
param azureBastionSubnetPrefix string
@description('CIDR prefix reserved for Azure Firewall subnet.')
param azureFirewallSubnetPrefix string
@description('CIDR prefix for Azure Application Gateway subnet.')
param azureAppGatewaySubnetPrefix string
@description('CIDR prefix allocated to jumpbox subnet.')
param jumpboxSubnetPrefix string
@description('CIDR prefix reserved for Container Apps environment subnet.')
param acaEnvironmentSubnetPrefix string
@description('CIDR prefix for DevOps build agent subnet.')
param devopsBuildAgentsSubnetPrefix string
@description('CIDR prefix dedicated to PostgreSQL database subnet.')
param psqlSubnetPrefix string
...
@minLength(1)
param aiFoundrySearchServiceName string

@description('Name of the Azure Cosmos DB account used by AI Foundry.')
@minLength(1)
param aiFoundryCosmosDbName string
...
module containerEnv 'br/public:avm/res/app/managed-environment:0.11.3' = if (deployContainerEnv) {
  ...
  params: {
    ...
    zoneRedundant: useZoneRedundancy && networkIsolation
    ...
    infrastructureSubnetResourceId: networkIsolation ? _caEnvSubnetId : null
